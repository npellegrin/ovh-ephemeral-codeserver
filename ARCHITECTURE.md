...
- **OVH API app credentials** (application key/secret/consumer key): the
  `ovh` provider, to create the least-privilege backups user via OVH's
  project-user API (`terraform-ephemeral`) and, optionally, DNS records.
...
`terraform-ephemeral/iam.tf` uses `ovh_cloud_project_user` (role
`objectstore_operator`). rclone (Ansible) authenticates to Swift with that
user's generated username/password.

`objectstore_operator` is scoped to the whole project, every container
including both Terraform state buckets, not just the backups one: OVH has
no per-container role. The user lives in `terraform-ephemeral`, not
`terraform-bootstrap`, so it's destroyed and recreated every cycle instead
of living forever; a leaked credential stops working after at most one
cycle. Its outputs are only valid after `terraform apply`, so the
Makefile's `generate-swift-vars` target (unlike `generate-vault-vars`)
runs after apply, not before.

The backups container is created Swift-side
(`openstack_objectstorage_container_v1`). OVH's S3 gateway doesn't expose
Swift-created containers, so rclone uses `type = swift`. Object Storage
holds full tarballs of the dev user's home directory, so treat the
credential as sensitive regardless of its short lifetime. `backup.yml` /
`restore.yml` pass it to rclone via `RCLONE_CONFIG_OVH_*` environment
variables, not a config file, so it never touches the instance's disk.
...
`make` regenerates these on every run. None are committed:

- `terraform-ephemeral/generated.tfvars`: `ovh_project_id`,
  `compute_region`, `keypair_name`, read from `terraform-bootstrap`'s
  outputs and passed via `-var-file` so they can't drift from bootstrap.
- `ansible/group_vars/vars.yml` (plaintext): `backup_bucket`,
  `swift_region`, `swift_tenant_id`, read from `terraform-bootstrap`'s
  outputs by `generate-vault-vars`.
- `ansible/group_vars/vault.yml` (encrypted): `code_server_password`,
  freshly generated by `generate-vault-vars` (rotates every run since the
  instance is rebuilt anyway); `swift_user` / `swift_key`, read from
  `terraform-ephemeral`'s outputs by `generate-swift-vars` after that
  stack's `apply` (the backup user doesn't exist before then).
...
- Swift credentials on the instance only ever exist as process environment
  variables during backup/restore, never written to disk, and the backup
  user itself is destroyed and recreated every create/destroy cycle
